...
def get_ddicts(label_fmt, im_root, ann_root=None, pattern='*', dataset_class=None):
    ...
    label_fmt: str
        Format of ground truth instance labels for each image. See above for detailed description. Options are:
          ...
          'rle: data_dicts will be read from list of rle masks
    ...
    im_root: str, Path object.
        ...
        If label == 'rle', im_root is the path to a json file containing a list of dictionaries of form
        {
            'file_name': <relative_path_to_image>, 
            'segmentation': # (from pycocotools.mask.encode())
            { 
                'size': [<int mask_rows>, <int mask_cols>],
                'counts': <utf-8 decoded string>
            }
        }
        Image directory should be in the same path as im_root.

        Otherwise, im_root is the path to a folder containing images to be loaded in the dataset.

    ann_root: str, Path object, or None
        Only needed if label_fmt != 'via2' or 'rle'
    ...
    if label_fmt.lower() in ['binary', 'label']:
        ...
        for idx, p in enumerate(data):
            ...
            annotations = []
            for mask in ann:
                d = RLE.decode(mask)  # decode to get bounding boxes
                bbox = extract_boxes(d)[0]
